...
The full SALT dataset is hosted on Zenodo. The download link is currently:
**UNDER REVIEW**
...
These scripts reproduce the place recognition benchmark results presented in the paper. They are designed to evaluate the recall performance of different place recognition algorithms across the dataset's scenarios.

* **benchmark_scan_context.py -** Evaluates the classic Scan Context algorithm using the provided KITTI formatted data for a specified scenario.

  ```
  python benchmarking/benchmark_scan_context.py --dataset_root /path/to/spot_autowalk_long_term_dataset --scenario indoor
  ```

* **benchmark_minkloc.py -** Evaluates MinkLoc3Dv2 for place recognition on the provided KITTI formatted data. This requires cloning the [MinkLoc3Dv2 repository](https://github.com/jac99/MinkLoc3Dv2) and providing the paths to the repo and the trained weights.

  ```
  python benchmarking/benchmark_minkloc.py --dataset_root /path/to/spot_autowalk_long_term_dataset --scenario indoor --weights /path/to/weights.pth --minkloc_dir /path/to/MinkLoc3Dv2
  ```
...
This software is provided as a research prototype and is not production-quality software. Please note that the code may contain missing features, bugs, and errors. The Mechatronic and Robotic Systems Laboratory does not guarantee maintenance or support for this software.
